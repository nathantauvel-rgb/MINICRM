...
export default function QuoteForm({ clients, initial, action, submitLabel }: Props) {
    ...
    return (
      <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center">
        <p className="text-sm text-slate-600">
          Vous devez d&apos;abord ajouter au moins un client avant de créer un devis.
        </p>
        <Link
    ...
    <form action={onSubmit} className="space-y-6">
      <Section title="Informations générales">
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="space-y-1.5">
            <label htmlFor="valid_until" className="text-sm font-medium text-slate-700">
              Valide jusqu&apos;au
            </label>
            <input
              id="valid_until"
              name="valid_until"
              type="date"
              defaultValue={initial?.valid_until ?? inDays(30)}
              className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
            />
          </div>
        </div>
