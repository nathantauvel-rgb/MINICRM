...
const styles = StyleSheet.create({
  ...
  legal: { marginTop: 16, fontSize: 8, color: "#64748b", lineHeight: 1.4 },
  footer: {
    position: "absolute",
    bottom: 24,
    left: 40,
...
export function InvoicePDF({ data }: { data: InvoiceData }) {
  const { settings, client, items, number, issue_date, due_date, paid_at, notes, total_ht } = data;
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          <View>
            <Text style={styles.brand}>{settings?.company_name || "Mon entreprise"}</Text>
          ...
          <View style={styles.meta}>
            ...
            <Text style={[styles.text, { marginTop: 8 }]}>Émise le : {formatDate(issue_date)}</Text>
            <Text style={styles.text}>Échéance : {formatDate(due_date)}</Text>
            {paid_at && (
              <Text style={styles.paidStamp}>PAYÉE le {formatDate(paid_at)}</Text>
            )}
          </View>
        </View>
        ...
        <View style={styles.twoCol}>
          ...
          <View style={styles.block}>
            ...
            <Text style={[styles.text, styles.bold]}>{client.company || client.name}</Text>
        ...
        <View style={styles.totalsBox}>
          ...
          <View style={styles.totalGrand}>
            <Text style={styles.totalGrandLabel}>Net à payer</Text>
            <Text style={styles.totalGrandValue}>{formatEUR(total_ht)}</Text>
          </View>
        </View>

        {(settings?.iban || settings?.payment_terms_days) && (
          <View style={styles.paymentBox}>
            <Text style={styles.paymentTitle}>Modalités de paiement</Text>
            {settings?.payment_terms_days != null && (
              <Text style={styles.text}>
                Paiement à réception de facture, à régler avant le {formatDate(due_date)}.
              </Text>
            )}
            {settings?.iban && (
              <Text style={styles.text}>Par virement bancaire — IBAN : {settings.iban}</Text>
            )}
          </View>
        )}
        ...
        <View style={styles.legal}>
          ...
          <Text>
            En cas de retard de paiement, application de pénalités au taux annuel de
            10 % et d&apos;une indemnité forfaitaire pour frais de recouvrement de
            40 € (art. L.441-10 et D.441-5 du Code de commerce). Pas d&apos;escompte
            pour paiement anticipé.
          </Text>
        </View>

        <Text style={styles.footer}>
          Facture {number} · {settings?.company_name || ""}
        </Text>
      </Page>
    </Document>
  );
